...
export const GigListing = ({
  ...
  return (
    <Link href={`/gigs/${id}`}>
      <div className="w-100 bg-black text-white rounded p-10 mt-4 hover:opacity-50 hover:cursor-pointer flex flex-wrap justify-between border-2" style={{ borderColor: `light${statecolor[status]}` }}>
        ...
        <div className="flex items-center lg:w-1/2 xl:w-1/3 sm:w-2/3 md:mt-0 sm:mt-5 justify-between flex-wrap pr-5">
          <div className="flex flex-col items-center xl:w-1/3">
            <div className={`text-2xl font-bold`} style={{ color: `light${statecolor[status]}` }}> {states[status]}</div>
